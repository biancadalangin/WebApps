...
if (isset($_POST['signup'])) {
    ...
    if ($errors == []){
        ...
        if(User::checkEmail($email) === true) {
            $_SESSION['errors_signup'] = ['This email is already taken'];
            header('location: ../index.php')  ;
        } else if (User::checkUserName($username) === true) {
            $_SESSION['errors_signup'] = ['This username is already taken'];
            header('location: ../index.php')  ;
        } else if (!preg_match("/^[a-zA-Z0-9_]*$/" , $username)) {
            $_SESSION['errors_signup'] = ['Only characters and numbers are allowed'];
            header('location: ../index.php')  ;
        } else {
